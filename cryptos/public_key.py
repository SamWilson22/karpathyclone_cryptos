...
def extended_euclidean_algorithm(a, b):
    ...
    This function implements the extened Euclidean
    alforithm and runs in O(log b) in the worst case,
...
class Curve:
    """
    Elliptic Curve over the field over integers modulo a prime.
    Points on the curve satisy y^2 = x^2 + a*x + b (mod p).
    """
    p: int 
    a: int 
    b: int
...
@dataclass
class Point:
    """ An interger point (x,y) on a Curve """
    curve: Curve
    x: int
    y: int
    ...
    def __add__(self,other):
        ...
        if self.x == other.x: # (self.y = other.y is guaranteed too per above check)
            m = (3 * self.x**2 + self.curve.a) * inv(2 * self.y, self.curve.p) # derivative of elliptical curve ? 
        else:
...
if __name__ == '__main__':
    ...
    print('x:', format(public_key.x, '064x').upper()) # (striop the 0x part denoting hex numher)
